...
todos:
  - id: analyze-renderer
    content: Analyze `renderer/renderer.js` and propose a refactoring strategy for modularization.
    status: pending
  - id: analyze-server
    content: Analyze `server/server.js` and propose a refactoring strategy for route extraction.
    status: pending
  - id: audit-security
    content: Audit IPC communication and Content Security Policy (CSP).
    status: pending
  - id: review-supabase
    content: Review Supabase migration files and error handling.
    status: pending
  - id: analyze-tests
    content: Identify gaps in E2E and integration tests.
    status: pending
isProject: false
---
